...
olcConsoleGameEngine::olcConsoleGameEngine()
{
	...
	void olcConsoleGameEngine::DrawCircle(int xc, int yc, int r, wchar_t c, short col)
	{
		int x = 0;
		int y = r;
		int p = 3 - 2 * r;
	...
	void olcConsoleGameEngine::FillCircle(int xc, int yc, int r, wchar_t c, short col)
	{
	...
	void olcConsoleGameEngine::DrawWireFrameModel(const vector<pair<float, float>>& vecModelCoordinates, float x, float y, float r, float s, short col)
	{
	...
	int olcConsoleGameEngine::ScreenWidth()
	{
		return m_nScreenWidth;
	}

	int olcConsoleGameEngine::ScreenHeight()
	{
...
bool olcConsoleGameEngine::OnUserDestroy()
	{
		return true;
	}
...
olcConsoleGameEngine::sKeyState olcConsoleGameEngine::GetKey(int nKeyID) { return m_keys[nKeyID]; }
	...
	olcConsoleGameEngine::sKeyState olcConsoleGameEngine::GetMouse(int nMouseButtonID) { return m_mouse[nMouseButtonID]; }
	bool olcConsoleGameEngine::IsFocused() { return m_bConsoleInFocus; }



	int olcConsoleGameEngine::Error(const wchar_t* msg)
	{
		wchar_t buf[256];
	...
	BOOL olcConsoleGameEngine::CloseHandler(DWORD evt)
	{
