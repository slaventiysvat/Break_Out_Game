...
//atomic<bool> m_bAtomActive = false;
...
GameBaseClass::GameBaseClass()
{
	...
	int GameBaseClass::ConstructConsole(int width, int height, int fontw, int fonth)
	{
    ...
    void GameBaseClass::Draw(int x, int y, wchar_t c, short col)
	{
	...
	void GameBaseClass::Fill(int x1, int y1, int x2, int y2, wchar_t c, short col)
	{
		Clip(x1, y1);
		Clip(x2, y2);
		for (int x = x1; x < x2; x++)
	...
	void GameBaseClass::DrawString(int x, int y, wstring c, short col)
	{
	...
	void GameBaseClass::DrawStringAlpha(int x, int y, wstring c, short col)
	{
		for (size_t i = 0; i < c.size(); i++)
		{
			if (c[i] != L' ')
	...
	void GameBaseClass::Clip(int& x, int& y)
	{
		if (x < 0) x = 0;
	...
	void GameBaseClass::DrawLine(int x1, int y1, int x2, int y2, wchar_t c, short col)
	{
	...
	void GameBaseClass::DrawCircle(int xc, int yc, int r, wchar_t c, short col)
	{
		int x = 0;
		int y = r;
		int p = 3 - 2 * r;
	...
	void GameBaseClass::FillCircle(int xc, int yc, int r, wchar_t c, short col)
	{
	...
	void GameBaseClass::DrawSprite(int x, int y, SpriteClass* sprite)
	{
		if (sprite == nullptr)
			return;
	...
	void GameBaseClass::DrawPartialSprite(int x, int y, SpriteClass* sprite, int ox, int oy, int w, int h)
	{
		if (sprite == nullptr)
			return;
	...
	void GameBaseClass::DrawWireFrameModel(const vector<pair<float, float>>& vecModelCoordinates, float x, float y, float r, float s, short col)
	{
	...
	void GameBaseClass::Start()
	{
		...
		// Star the thread
		thread t = thread(&GameBaseClass::GameThread, this);

		// Wait for thread to be exited
		t.join();
	}

	int GameBaseClass::ScreenWidth()
	{
		return m_nScreenWidth;
	}

	int GameBaseClass::ScreenHeight()
	{
		return m_nScreenHeight;
	}

	void GameBaseClass::GameThread()
	{
...
bool GameBaseClass::OnUserDestroy()
	{
		return true;
	}


GameBaseClass::sKeyState GameBaseClass::GetKey(int nKeyID) { return m_keys[nKeyID]; }
	int GameBaseClass::GetMouseX() { return m_mousePosX; }
	int GameBaseClass::GetMouseY() { return m_mousePosY; }
	GameBaseClass::sKeyState GameBaseClass::GetMouse(int nMouseButtonID) { return m_mouse[nMouseButtonID]; }
	bool GameBaseClass::IsFocused() { return m_bConsoleInFocus; }



	int GameBaseClass::Error(const wchar_t* msg)
	{
		wchar_t buf[256];
	...
	BOOL GameBaseClass::CloseHandler(DWORD evt)
	{
...
//atomic<bool> GameBaseClass::m_bAtomActive = false;
//condition_variable GameBaseClass::m_cvGameFinished;
//mutex GameBaseClass::m_muxGame;
